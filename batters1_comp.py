...
class Batter_comp():
    ...
    def calculate(self,leagues,overs1,BowlingType,Season,limit):
            ...
            for player in players:
                ...
                balls=len(self.df.loc[(self.df['is_wide']==0)  & (self.df['is_noball']==0) & (self.df["LeagueName"].isin(leagues))  & (self.df["BowlingType"].isin(BowlingType)) & (self.df["overs"].isin(overs))  & (self.df["Season"].isin(Season)) & (self.df['BattingTeam'] == player) ] )
                out = len(   self.df.loc[(self.df["BattingTeam"] == player) & (self.df["LeagueName"].isin(leagues))  & (self.df["player_out"]==1)  & (self.df["BowlingType"].isin(BowlingType))   & (self.df["overs"].isin(overs)) & (self.df["Season"].isin(Season))])
...
# # print(r1)
# with open('C:/Users/adith/Documents/ds/t20_leagues/set_3_leagues/app/teams/batting_comp/batting_comp.pkl', 'wb') as f:
